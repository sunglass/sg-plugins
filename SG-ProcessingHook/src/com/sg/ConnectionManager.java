...
public class ConnectionManager {
	...
	public ArrayList<Project> getProjects() throws Exception {
		...
		JSONObject jsonProjects = this.httpGetRequestURL("projects");

		try {
			JSONArray projectArray = new JSONArray(jsonProjects.get("projects")
					.toString());
			for (int i = 0; i < projectArray.length(); i++) {
				JSONObject row = projectArray.getJSONObject(i);
